#!/usr/bin/env python3
"""
Final Working CrewAI Multi-Agent Interview Question Generator
This version ensures all agents work properly with the correct model format.
Each agent generates exactly 10 questions for their domain.
"""
...
try:
    ...
    print(f"❌ Error initializing Gemini Pro: {str(e)}")
    print("\nTROUBLESHOOTING STEPS:")
    print("1. Verify GOOGLE_API_KEY in .env file is correct")
...
def run_interview(candidate_name, candidate_role):
    ...
    try:
        print("🔍 Creating interview crew...")
        interview_crew = create_interview_crew(candidate_name, candidate_role)
        
        if not interview_crew:
            raise ValueError("Failed to create interview crew")
            
        print("✅ Interview crew created successfully")
        print("🚀 Starting interview process...")
        
        try:
            result = interview_crew.kickoff()
            print("✅ Interview process completed successfully")
        except Exception as e:
            print(f"❌ Error during interview process: {str(e)}")
            print("\nTROUBLESHOOTING STEPS:")
            print("1. Check your internet connection")
            print("2. Verify your Google API key has sufficient quota")
            print("3. Try reducing the number of questions or agents")
            raise
            ...
            try:
                if hasattr(task, 'output') and task.output:
                    ...
                    if i == 0:  # Aptitude task
                        ...
                        print(f"✅ Aptitude questions: {count_questions(output_text)} questions")
                    elif i == 1:  # Soft skills task
                        agent_outputs['soft_skills_questions'] = output_text
                        print(f"✅ Soft skills questions: {count_questions(output_text)} questions")
                    elif i == 2:  # Coding task
                        agent_outputs['coding_questions'] = output_text
                        print(f"✅ Coding questions: {count_questions(output_text)} questions")
                    elif i == 3:  # HR task
                        agent_outputs['hr_questions'] = output_text
                        print(f"✅ HR questions: {count_questions(output_text)} questions")
            except Exception as e:
                print(f"⚠️ Error processing task {i}: {str(e)}")
        ...
        return {
            ...
            'status': 'success',
            'crew_result': str(result)[:500] + '...' if result and len(str(result)) > 500 else str(result)
        }
...
def test_llm_connection():
    ...
    print("🔍 Testing LLM Connection...")
    try:
        # Test using the LLM directly for connection test
